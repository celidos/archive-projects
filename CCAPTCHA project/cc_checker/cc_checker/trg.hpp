#ifndef TRG_HPP_INCLUDED
#define TRG_HPP_INCLUDED
...
std::vector<Rect> res;              // Сюда будет записываться результат - найденные текстовые блоки
//---------------------------------------------------------------------------------------------
const int dw = 24;
const int dh = 24;
const int h_min = 3;
...
void build_b() {
    int ww = w / dw;
    int hh = h / dh;
    std::vector<short> temp (w + 3, 0);
    std::vector<int>   temp2(w + 3, 0);
    b.assign(h + 1, temp);
    for (int dy = 0; dy <= dh / 2; dy += dh / 2) {
        std::vector <std::vector<int>> m(hh + 1, temp2);
